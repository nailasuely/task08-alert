...
// cnfiguração do barramento I2C para o display OLED
...
bool ambiente_quente = false; // Flag 
...
uint16_t centro_x = 0;
uint16_t centro_y = 0;
bool centro_definido = false;  // Flag 
...
// Função para exibir informações no display de forma organizada
void exibir_informacoes(uint16_t adc_x, uint16_t adc_y, uint16_t tempo_joystick_inativo, uint8_t nivel_alerta, bool ambiente_quente, int temperatura) {
    ...
    ssd1306_draw_string(&tela, "Sistema de Alerta", 10, 8);
    ssd1306_draw_string(&tela, str_x, 10, 20);
    //ssd1306_draw_string(&tela, str_y, 10, 30);
    //ssd1306_draw_string(&tela, str_joystick, 10, 40);
...
int main() {
    ...
    while (true) {
        ...
        if (ambiente_quente) {
            // Em ambiente quente, os tempos de inatividade são acelerados
            ...
            // Em ambiente normal, os tempos de inatividade são normais
        ...
        // Verifica se a temperatura está muito alta e emite um alerta de perigo
        if (temperatura > 40) {
            iniciar_alerta(3);  // Emergência
            printf("PERIGO! Temperatura muito alta: %d C\n", temperatura);
        } else if (temperatura > 30) {
            iniciar_alerta(2);  // Alerta Médio
            printf("Alerta! Temperatura alta: %d C\n", temperatura);
        }
